...
-- | Cache an action, producing a version of this action that is cached
-- for 'interval' seconds. The cache begins uninitialized.
--
-- The outer 'MonadIO' is responsible for setting up the cache. Use the inner one to
...
secondsPassed
    :: NominalDiffTime -- ^ Seconds
    -> UTCTime -- ^ Start time
    -> UTCTime -- ^ End time
    -> Bool
secondsPassed interval start end = addUTCTime interval start > end

-- | Cache an action, The cache begins uninitialized.
--
-- The outer 'MonadIO' is responsible for setting up the cache. Use the inner one to
...
cachedIOWith
    ...
    --   the cache is considered still fresh and returns the cached action
    -> t a -- ^ action to cache
    -> m (t a)
